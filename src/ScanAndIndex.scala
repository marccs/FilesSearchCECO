...
case class StartFileScanner(m : Monitor)
...
class NewIndexerException(val word : String, val path_filename : String) extends ConcurrentException
...
class FileScanner(mainActor : Actor) extends Actor with ExceptionModel {
